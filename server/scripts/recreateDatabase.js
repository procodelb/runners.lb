...
const initDatabase = async () => {
  try {
    ...
    await runQuery('CREATE INDEX idx_clients_business_name ON clients(business_name)');
    await runQuery('CREATE INDEX idx_clients_phone ON clients(phone)');
    await runQuery('CREATE INDEX idx_orders_order_ref ON orders(order_ref)');
    await runQuery('CREATE INDEX idx_orders_status ON orders(status)');
    await runQuery('CREATE INDEX idx_transactions_type ON transactions(type)');
    await runQuery('CREATE INDEX idx_transactions_created_at ON transactions(created_at)');
    await runQuery('CREATE INDEX idx_transactions_actor ON transactions(actor_type, actor_id)');
    await runQuery('CREATE INDEX idx_cashbox_driver_id ON cashbox(driver_id)');
    ...
    await runQuery(`
      INSERT INTO price_list (country, area, fees_lbp, fees_usd) VALUES 
